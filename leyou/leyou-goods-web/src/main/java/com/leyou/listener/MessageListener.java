...
public class MessageListener {
    ...
    public void editThymeleafData(Long spuId) throws Exception {
        System.out.println("开始监听修改Thymeleaf数据,supId="+spuId);
        if(spuId==null){
           return;
        }
        goodService.creatHtml(spuId);
        System.out.println("结束修改监听修改Thymeleaf数据,supId="+spuId+"修改成功");
    ...
    public void deleteThymeleafData(Long spuId) throws Exception {
        System.out.println("开始监听删除Thymeleaf数据,supId="+spuId);
        if(spuId==null){
            return;
        }
        goodService.deleteHtml(spuId);
        System.out.println("结束监听删除Thymeleaf数据,supId="+spuId+"删除成功");
